...
class NetworkClassifier(FlakeFighter):
    ...
    def flaky_test_live(self, execution):
        raise NotImplementedError(
            "NetworkClassifier only supports postprocessing classification"
        )
    ...
    def _longest_passed_duration(self, test) -> Optional[float]:
        ...
        for execution in test.executions:
            ...
            if (
                execution.start_time is None
                or execution.end_time is None
            ):
                continue
    ...
    def _report_confirms_socket_blocked(
        self,
        test_report: dict[str, Any],
    ) -> bool:
        ...
        return any(
            name in text
            for name in SOCKET_BLOCKED_EXCEPTION_NAMES
        )

    def _report_confirms_timeout(
        self,
        test_report: dict[str, Any],
    ) -> bool:
        return (
            TIMEOUT_FAILURE_MARKER
            in self._report_text(
                test_report
            )
        )
    ...
    def _store_blocked_results(
        ...
        for test in eligible_tests:
            ...
            if self._report_confirms_timeout(
                report
            ):
                continue
            ...
            result = FlakefighterResult(
                ...
                flaky=network_sensitive,
            )

            if (
                result
                not in test.flakefighter_results
            ):
